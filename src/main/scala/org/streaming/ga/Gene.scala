...
private[ga] class Gene[U] private (
  ...
  def unapply(bitSet: java.util.BitSet): (U, Operator) = (
    quantization.toU(BitsOperator.convert(encoding.rValue, bits)),
    op(BitsOperator.convert(encoding.rOp, bits))
  )
  ...
  def xOver(that: Gene[U], geneIndexer: GeneIndexer): Gene[U] = {
    val clonedBits = BitsOperator.duplicate(bits)
  ...
  def mutate(geneIndexer: GeneIndexer): Gene[U] = {
    val clonedBits = BitsOperator.duplicate(bits) // flip the bit
...
private[ga] object Gene{
  ...
  class Encoding(nValueBits: Int, nOpBits: Int){
    ...
    val length: Int = nValueBits + nOpBits
