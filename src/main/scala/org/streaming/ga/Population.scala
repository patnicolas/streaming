...
import org.streaming.ga.Chromosome.ChomosomesPool
...
private[ga] class Population[U, T <: Gene[U]] private (
  ...
  def getChromosomes: ChomosomesPool[U, T] = chromosomes
  ...
  def select(score: Chromosome[U, T] => Unit, cutOff: Double): Unit = {
    ...
    val cumul = chromosomes./:(0.0)((s, c) => s + c.getCost) / ScalingFactor
