...
import org.pipeline.plots.Plot.getLocation
...
abstract class Plot(legend: Legend, theme: PlotTheme) {
  ...
  def stroke(index: Int): Stroke = strokeList(index % strokeList.size)
  ...
  protected def createFrame(id: String, chart: JFreeChart): Unit = {
    val frame = new ChartFrame(s"Chart ${Plot.count + 1}: $id", chart)
