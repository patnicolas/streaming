...
import org.pipeline.plots.Plot.{validateDisplay, validateDisplaySize}
...
final class ScatterPlot(legend: Legend, theme: PlotTheme) extends Plot(legend, theme) {
  ...
  def display(
    xy1: Vector[(Double, Double)],
    xy2: Vector[(Double, Double)],
    width: Int,
    height: Int): Boolean = {
    require(
      xy1.nonEmpty,
    ...
    if (validDisplay) {
      ...
      renderer2.setSeriesShape(0, ShapeUtils.createDiamond(3.0F))
  ...
  def display(xs: OBS, lbls: List[String], w: Int, h: Int): Boolean = {
    ...
    if (validDisplay) {
      ...
      val shapes = Array[Shape](
        ShapeUtils.createDiamond(3.0F),
        ShapeUtils.createRegularCross(2.0F, 2.0F)
      )
...
object ScatterPlot {
  ...
  private val strokesList: immutable.List[Stroke] = immutable.List[Stroke](
